...
def init_ik_target_to_current_ee_pose(
    env: "ManagerBasedEnv",
    env_ids: torch.Tensor,
):
    """Initialize IK controller target to current EE pose at reset.
    
    This prevents the IK controller from having a zero target at the first step,
    which would cause immediate large motions (drift) even with zero action input.
    ...
    # Find the IK action term (has _ik_controller attribute)
    for term in action_manager._terms.values():
        if hasattr(term, '_ik_controller'):
            # Compute current EE pose using the action term's own method
            ee_pos_curr, ee_quat_curr = term._compute_frame_pose()
            
            # Set IK controller's target to current pose for the reset envs
            term._ik_controller.ee_pos_des[env_ids] = ee_pos_curr[env_ids]
            term._ik_controller.ee_quat_des[env_ids] = ee_quat_curr[env_ids]
            
            # Also store as fixed target for FixedPoseIKAction (if used)
            if hasattr(term, '_fixed_ee_pos') and term._fixed_ee_pos is not None:
                term._fixed_ee_pos[env_ids] = ee_pos_curr[env_ids]
                term._fixed_ee_quat[env_ids] = ee_quat_curr[env_ids]
            break


def set_default_tm7s_arm_pose(
    env: "ManagerBasedEnv",
    env_ids: torch.Tensor,
    default_arm_pose: list[float],                 # 長度=6
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
    arm_joint_names: list[str] = TM7S_ARM_JOINTS,
):
    asset: Articulation = env.scene[asset_cfg.name]

    # 取得目前的 default joint (形狀: [num_envs, num_joints_total])
    # 用 clone 避免 reference 問題
    cur = asset.data.default_joint_pos.clone()

    arm_joint_ids, _ = asset.find_joints(arm_joint_names)
    if len(arm_joint_ids) != len(default_arm_pose):
        raise RuntimeError(
            f"Arm joint count mismatch. found={len(arm_joint_ids)} pose_len={len(default_arm_pose)}"
        )

    arm_joint_ids = torch.as_tensor(arm_joint_ids, device=env.device, dtype=torch.long)
    pose = torch.tensor(default_arm_pose, device=env.device, dtype=cur.dtype).view(1, -1)

    # 只更新指定 env_ids 的手臂 joints
    cur[env_ids[:, None], arm_joint_ids[None, :]] = pose.repeat(env_ids.shape[0], 1)

    # ✅ 關鍵：維度必須保持 num_joints_total，不要變成 6 或 7
    asset.data.default_joint_pos = cur

    # ✅ 關鍵：實際寫入模擬，不只是更新 default_joint_pos
    # Get the full joint state for the specified envs
    joint_pos = cur[env_ids].clone()
...
def randomize_tm7s_joint_by_gaussian_offset(
    env: "ManagerBasedEnv",
    env_ids: torch.Tensor,
    mean: float,
    std: float,
    ...
    # 取出「整個機器人」的 joint state (N, num_joints_total)
    joint_pos = asset.data.default_joint_pos[env_ids].clone()
    joint_vel = asset.data.default_joint_vel[env_ids].clone()

    # 找到手臂關節 id（在整個 joint vector 內的 index）
    arm_joint_ids, _ = asset.find_joints(arm_joint_names)
    if len(arm_joint_ids) == 0:
        raise RuntimeError(f"Cannot find arm joints with names: {arm_joint_names}")

    arm_joint_ids = torch.as_tensor(arm_joint_ids, device=env.device, dtype=torch.long)

    # 只對手臂關節加 gaussian noise
    joint_pos[:, arm_joint_ids] += math_utils.sample_gaussian(
        mean, std, (joint_pos.shape[0], arm_joint_ids.numel()), joint_pos.device
    )

    # 只對手臂關節做 joint limit clamp
    joint_pos_limits = asset.data.soft_joint_pos_limits[env_ids]  # (N, num_joints_total, 2)
    low = joint_pos_limits[:, arm_joint_ids, 0]
    high = joint_pos_limits[:, arm_joint_ids, 1]
    joint_pos[:, arm_joint_ids] = joint_pos[:, arm_joint_ids].clamp_(low, high)
...
# ===========================
# Alias functions for compatibility with config files
# ===========================
...
def set_default_joint_pose(
    ...
    """
    Alias for set_default_tm7s_arm_pose with a compatible signature.
    Sets default joint pose for the specified joints.
    """
    if joint_names is None:
        joint_names = TM7S_ARM_JOINTS
    return set_default_tm7s_arm_pose(
        env=env,
        env_ids=env_ids,
        default_arm_pose=default_pose,
        asset_cfg=asset_cfg,
