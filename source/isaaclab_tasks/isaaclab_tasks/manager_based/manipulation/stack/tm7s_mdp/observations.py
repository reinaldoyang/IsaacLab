...
def tm7s_gripper_pos(
    ...
    if len(joint_ids) != 1:
        raise RuntimeError(f"Cannot find drive joint '{drive_joint_name}' in robot articulation.")
    jid = joint_ids[0]

    q = robot.data.joint_pos[env_ids, jid].unsqueeze(-1)  # (N, 1)

    # normalize to [0, 1]
    denom = (close_q - open_q) if abs(close_q - open_q) > 1e-6 else 1.0
    g = (q - open_q) / denom
...
def _tm7s_like_gripper_closed_mask(
    ...
    if len(gripper_joint_ids) == 2:
        ...
        return (torch.abs(q0 - open_val) > thr) & (torch.abs(q1 - open_val) > thr)
